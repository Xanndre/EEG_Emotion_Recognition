CHANNELS_32 = ['FP1', 'AF3', 'F3', 'F7', 'FC5', 'FC1', 'C3', 'T7',
               'CP5', 'CP1', 'P3', 'P7', 'PO3', 'O1', 'OZ', 'PZ',
               'FP2', 'AF4', 'FZ', 'F4', 'F8', 'FC6', 'FC2', 'CZ',
               'C4', 'T8', 'CP6', 'CP2', 'P4', 'P8', 'PO4', 'O2']

CHANNELS_28 = ['F3', 'F7', 'FC5', 'FC1', 'C3', 'T7', 'CP5', 'P3', 'P7', 'PO3', 'O1',
               'OZ', 'PZ', 'FP2', 'AF4', 'FZ', 'F4', 'FC6', 'FC2', 'CZ', 'C4', 'T8',
               'CP6', 'CP2', 'P4', 'P8', 'PO4', 'O2']

CHANNELS_11 = ['C4', 'FC1', 'PO3', 'F4',
               'FC5', 'CZ', 'P7', 'O1', 'T8', 'CP2', 'O2']

CHANNELS_10 = ['C4', 'FC1', 'PO3', 'F4', 'FC5', 'CZ', 'P7', 'O1', 'T8', 'CP2']

CHANNELS_9 = ['C4', 'FC1', 'PO3', 'F4', 'FC5', 'CZ', 'O1', 'T8', 'CP2']

CHANNELS_8 = ['C4', 'FC1', 'PO3', 'F4', 'FC5', 'CZ', 'O1', 'T8']

CHANNELS_7 = ['C4', 'FC1', 'PO3', 'F4', 'FC5', 'CZ', 'T8']

CHANNELS_6 = ['C4', 'FC1', 'PO3', 'F4', 'FC5', 'CZ']

CHANNELS_4 = ['C4', 'FC1', 'PO3', 'F4']

CHANNELS_SEED = ['FP1', 'FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F5', 'F3',
                 'F1', 'FZ', 'F2', 'F4', 'F6', 'F8', 'FT7', 'FC5', 'FC3',
                 'FC1', 'FCZ', 'FC2', 'FC4', 'FC6', 'FT8', 'T7', 'C5',
                 'C3', 'C1', 'CZ', 'C2', 'C4', 'C6', 'T8', 'TP7', 'CP5', 'CP3',
                 'CP1', 'CPZ', 'CP2', 'CP4', 'CP6', 'TP8', 'P7', 'P5', 'P3',
                 'P1', 'PZ', 'P2', 'P4', 'P6', 'P8', 'PO7', 'PO5', 'PO3',
                 'POZ', 'PO4', 'PO6', 'PO8', 'CB1', 'O1', 'OZ', 'O2', 'CB2']

CHANNELS_50 = ['FP1', 'FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F5', 'F3',
               'F1', 'FZ', 'F2', 'F4', 'F6', 'F8', 'FC5', 'FC3'
               'FC1', 'FC2', 'FC6', 'T7', 'C5',
               'C3', 'C1', 'CZ', 'C2', 'C4', 'C6', 'T8', 'TP7', 'CP5', 'CP3',
               'CP1', 'CPZ', 'CP2', 'CP4', 'CP6', 'P7', 'P3',
               'P1', 'PZ', 'P2', 'P4', 'P8', 'PO3', 'PO4', 'CB1', 'O1', 'OZ', 'O2', 'CB2']

CHANNELS_42 = ['FP1', 'FP2', 'AF3', 'AF4', 'F7', 'F3',
               'FZ', 'F4', 'F8', 'FC5',
               'FC1', 'FC2', 'FC6', 'T7', 'C5',
               'C3', 'C1', 'CZ', 'C2', 'C4', 'C6', 'T8', 'TP7', 'CP5', 'CP3',
               'CP1', 'CPZ', 'CP2', 'CP4', 'CP6', 'P7', 'P3',
               'P1', 'PZ', 'P2', 'P4', 'P8', 'PO3', 'PO4', 'O1', 'OZ', 'O2']

CHANNELS_37 = ['FP1', 'FP2', 'AF3', 'AF4', 'F7', 'F3',
               'FZ', 'F4', 'F8', 'FC5',
               'FC1', 'FC2', 'FC6', 'T7',
               'C3', 'CZ', 'C2', 'C4', 'C6', 'T8', 'CP5',
               'CP1', 'CP2', 'CP4', 'CP6', 'P7', 'P3',
               'P1', 'PZ', 'P2', 'P4', 'P8', 'PO3', 'PO4', 'O1', 'OZ', 'O2']

BANDS = {'Delta': (1, 4),
         'Theta': (4, 8),
         'Alpha': (8, 14),
         'Beta': (14, 31),
         'Gamma': (31, 50)}

SEED_SESSION1_LABEL = [1, 2, 3, 0, 2, 0, 0, 1, 0, 1,
                       2, 1, 1, 1, 2, 3, 2, 2, 3, 3, 0, 3, 0, 3]
SEED_SESSION2_LABEL = [2, 1, 3, 0, 0, 2, 0, 2, 3, 3,
                       2, 3, 2, 0, 1, 1, 2, 1, 0, 3, 0, 1, 3, 1]
SEED_SESSION3_LABEL = [1, 2, 2, 1, 3, 3, 3, 1, 1, 2,
                       1, 0, 2, 3, 3, 0, 2, 3, 0, 0, 2, 0, 1, 0]
